...
pub fn mix_tricopter(
    ...
    if params.overshoot > 0.0 {
        // rear motor is saturated, so reduce its output to range.max and reduce front motors similarly
    ...
    params.undershoot = (outputs[FL]).min(outputs[FR]) - range.min;
    if params.undershoot < 0.0 {
        outputs[REAR] = range.min;
...
pub fn mix_quad_x(
    ...
    if commands.yaw > 0.0 {
        // check if m1 or m2 will have output less than range.min
        params.undershoot = (outputs[1] - range.min).min(params.undershoot);
        params.undershoot = (outputs[2] - range.min).min(params.undershoot);
        // check if m0 or m3 will have output greater than range.max
        ...
        // check if m0 or m3 will have output less than range.min
        params.undershoot = (outputs[0] - range.min).min(params.undershoot);
        params.undershoot = (outputs[3] - range.min).min(params.undershoot);
        // check if m1 or m2 will have output greater than range.max
...
pub fn mix_hex_x(commands: MotorMixerCommands, range: MotorOutputRange, params: &mut MotorMixerParameters) -> [f32; 6] {
    ...
    outputs[0] = outputs[0].clamp(range.min, range.max);
    outputs[1] = outputs[1].clamp(range.min, range.max);
    outputs[2] = outputs[2].clamp(range.min, range.max);
    outputs[3] = outputs[4].clamp(range.min, range.max);
    ...
    if commands.roll > 0.0 {
        // check if m2, m3, or m5 will have output less than range.min
        params.undershoot = (outputs[2] - range.min).min(params.undershoot);
        params.undershoot = (outputs[3] - range.min).min(params.undershoot);
        params.undershoot = (outputs[5] - range.min).min(params.undershoot);
        // check if m0, m1, or m4 will have output greater than range.max
        params.overshoot = (outputs[0] - range.max).max(params.overshoot);
        params.overshoot = (outputs[1] - range.max).max(params.overshoot);
        params.overshoot = (outputs[4] - range.max).max(params.overshoot);
    ...
    //#if !defined(LIBRARY_MOTOR_MIXERS_USE_NO_OVERFLOW_CHECKING_YAW)
    if commands.yaw > 0.0 {
        // check if m0, m1, or m5 will have output less than range.min
        params.undershoot = (outputs[0] - range.min).min(params.undershoot);
        params.undershoot = (outputs[1] - range.min).min(params.undershoot);
        params.undershoot = (outputs[5] - range.min).min(params.undershoot);
        // check if m2, m3, or m4 will have output greater than range.max
        params.overshoot = (outputs[2] - range.max).max(params.overshoot);
        params.overshoot = (outputs[3] - range.max).max(params.overshoot);
        params.overshoot = (outputs[4] - range.max).max(params.overshoot);
        if commands.yaw + (params.undershoot - params.overshoot) > 0.0 {
        ...
        // check if m2, m3, or m4 will have output less than range.min
        params.undershoot = (outputs[2] - range.min).min(params.undershoot);
        params.undershoot = (outputs[3] - range.min).min(params.undershoot);
        params.undershoot = (outputs[4] - range.min).min(params.undershoot);
        // check if m0, m1, or m5 will have output greater than range.max
        params.overshoot = (outputs[1] - range.max).max(params.overshoot);
        params.overshoot = (outputs[3] - range.max).max(params.overshoot);
        params.overshoot = (outputs[5] - range.max).max(params.overshoot);
        if commands.yaw - (params.undershoot - params.overshoot) < 0.0 {
            params.throttle -= params.undershoot + params.overshoot;
            let yaw_delta = -params.undershoot - params.overshoot;
            outputs[0] -= yaw_delta;
